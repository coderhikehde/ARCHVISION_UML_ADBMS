...
import { ExplainUMLModal } from "@/components/features/explain-uml-modal";
    ...
    <>
      <div className="flex h-12 select-none items-center justify-between gap-2 overflow-x-auto border-b border-gray-200 bg-white px-4">
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          <span
            className="max-w-[180px] truncate text-[13.5px] font-semibold text-gray-900"
            title={diagramName}
          >
            {diagramName}
          </span>
          ...
          <DropdownMenu open={arrangeOpen} onOpenChange={setArrangeOpen}>
            ...
            <DropdownMenuContent align="start" className="w-56">
              ...
              {ALIGN_ITEMS.map((item) => (
                <DropdownMenuItem
                  key={item.axis}
                  onClick={() => dispatch("archvision:align", { axis: item.axis })}
                >
              ...
              <DropdownMenuItem
                onClick={() => dispatch("archvision:distribute", { axis: "horizontal" })}
              >
                <AlignHorizontalDistributeCenter className="h-3.5 w-3.5 text-gray-400" />
                Evenly (horizontal)
              </DropdownMenuItem>
              <DropdownMenuItem
                onClick={() => dispatch("archvision:distribute", { axis: "vertical" })}
              >
                <AlignVerticalDistributeCenter className="h-3.5 w-3.5 text-gray-400" />
                Evenly (vertical)
              </DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuLabel>Auto Layout</DropdownMenuLabel>
              <DropdownMenuItem
                onClick={() => dispatch("archvision:auto-layout", { direction: "LR" })}
              >
                <MoveRight className="h-3.5 w-3.5 text-gray-400" /> Left → Right
              </DropdownMenuItem>
              <DropdownMenuItem
                onClick={() => dispatch("archvision:auto-layout", { direction: "TB" })}
              >
              ...
              <DropdownMenuItem onClick={() => dispatch("archvision:reorder-participants")}>
                ...
                Reorder Participants (by message flow)
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
        ...
        <div className="flex items-center gap-1.5">
          ...
          {iconAction(
            "Comments",
            () => useEditorUI.getState().setCommentsOpen(true),
            <MessageSquare className="h-4 w-4" />,
            commentCount > 0
          )}

          {/* ── NEW: Explain + Report buttons ── */}
          <div className="h-6 w-px bg-gray-200" />
          {iconAction(
            "Explain this UML",
            () => setExplainOpen(true),
